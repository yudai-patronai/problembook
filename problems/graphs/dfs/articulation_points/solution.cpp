...
struct Graph {
    ...
    Graph(size_t n_) : n(n_), edges(n, std::vector<Edge>()) {}
...
struct APFinder {
    ...
    void DfsIt(size_t start) {
        // In fact it was probably easier to just modify the %rsp. Well, never mind.
        ...
        while (! stack.empty()) {
            ...
            if (to == g.n) {
                ...
                //std::cerr << "Enter DFS, node " << (node+1) << ", parent " << (parent+1) << "\n";
            } else {
            ...
            for (; i < g.edges[node].size(); ++i) {
                ...
                if (used[to]) {
                    ...
                    //std::cerr << "Following edge to child " << (to + 1) << " via pseudo-recursion\n";
