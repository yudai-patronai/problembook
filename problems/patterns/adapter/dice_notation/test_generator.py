...
def gen_res(dst):
    dice_lst = (dice_res(i) for i in dst.replace(" ", "").split("+"))
...
if __name__ == '__main__':
    ...
    for i in range(1,16):
        ...
        with open( os.path.join(test_folder, "{:02}".format(i)), "w") as f:
            f.write(tst)
        with open(os.path.join(test_folder, "{:02}.a".format(i)), "w") as f:
            f.write(gen_res(tst))
