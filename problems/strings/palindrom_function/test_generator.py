...
def generate_test(n, t, folder):
    ...
    with open(src, "w") as f:
        f.write(t[0])
    answer = solution.find_palindroms(t[0])
    if len(t) >= 2:
        assert answer == t[1], n
    if len(t[0]) < 1000:
        assert answer == slow_solution.find_palindroms(t[0]), n
...
if __name__ == "__main__":
    ...
    tests = [
        ...
        ("abababcb" * 12400,),
        ("gurugugu" * 10000,),
        (generate_simple("a", "xyz", 1000),),
        (generate_simple("aba", "xyz", 10000),),
        (generate_simple("abcba", "xyzyxyz", 100000),),
        (generate_simple(generate_simple("iki", "xyz", 5 + 6 * 3), "xyzyxyz", 100000),),
        (generate_simple(generate_simple("iki", "xyz", 5 + 6 * 3), "kixyzki", 100000),),
        (generate_simple(generate_simple(generate_simple("aba", "abcdef", 16 + 17 * 3), "xyz", 5 + 6 * (16 + 17 * 3)), "kixyzki", 100000),),
    ]
