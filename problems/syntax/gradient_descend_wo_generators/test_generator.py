...
tests = [
    ...
    ["1 - (1/math.sqrt(2*math.pi*2*2)*math.exp(-(x-5)*(x-5)/2/2/2))", "(math.exp(-1/8*(-5 + x)*(x-5))*(-5 + x))/(8*math.sqrt(2*math.pi))", 3, 0, 0],
    ["math.exp(x+4)+math.exp(-x-20)", "math.exp(x+4)-math.exp(-x-20)", 0, 0.01, 1e-3]
]
...
for j, (a, b, c, d, e) in enumerate(tests):
    ...
    with open(os.path.join(tests_dir, "{:02}.a".format(j+1)), "w") as f:
        if lr != 0.:
            xi, fxi = gradient_descent(fx, dx, x, lr, eps)
            f.write("{:10.5}{:10.5}\n".format(xi, fxi))
        else:
            xi, fxi = gradient_descent(fx, dx, x)
            f.write("{:10.5}{:10.5}\n".format(xi, fxi))
